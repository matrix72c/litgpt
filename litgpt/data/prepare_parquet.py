...
class ParquetDataRecipe(DataChunkRecipe):
    is_generator = True

    def __init__(self, tokenizer: Tokenizer, chunk_size: int, text_column: str = "content"):
    ...
    def prepare_item(self, item_metadata):
        ...
        try:
            ...
            # reduce RAM usage
            for batch in parquet_file.iter_batches(batch_size=8192, columns=[self.text_column]):
                for text in batch.to_pandas()[self.text_column]:
...
def prepare(
    input_dir: Path = Path("data/parquetdata"),
    ...
    data_recipe = ParquetDataRecipe(tokenizer=tokenizer, chunk_size=chunk_size, text_column=text_column)
